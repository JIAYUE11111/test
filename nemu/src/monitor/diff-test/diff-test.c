...
void difftest_step(uint32_t eip) {
  ...
  if(cpu.eax != r.eax){
    printf("EAX differ @ 0x%8x!\tnemu:\t%8x\tgt:\t%8x\n", cpu.eip, cpu.eax, r.eax);
    diff=true;
  }
  if(cpu.ebx != r.ebx){
    printf("EBX differ @ 0x%8x!\tnemu:\t%8x\tgt:\t%8x\n", cpu.eip, cpu.ebx, r.ebx);
    diff=true;
  }
  if(cpu.ecx != r.ecx){
    printf("ECX differ @ 0x%8x!\tnemu:\t%8x\tgt:\t%8x\n", cpu.eip, cpu.ecx, r.ecx);
    diff=true;
  }
  if(cpu.edx != r.edx){
    printf("EDX differ @ 0x%8x!\tnemu:\t%8x\tgt:\t%8x\n", cpu.eip, cpu.edx, r.edx);
    diff=true;
  }
  if(cpu.esp != r.esp){
    printf("ESP differ @ 0x%8x!\tnemu:\t%8x\tgt:\t%8x\n", cpu.eip, cpu.esp, r.esp);
    diff=true;
  }
  if(cpu.ebp != r.ebp){
    printf("EBP differ @ 0x%8x!\tnemu:\t%8x\tgt:\t%8x\n", cpu.eip, cpu.ebp, r.ebp);
    diff=true;
  }
  if(cpu.esi != r.esi){
    printf("ESI differ @ 0x%8x!\tnemu:\t%8x\tgt:\t%8x\n", cpu.eip, cpu.esi, r.esi);
    diff=true;
  }
  if(cpu.edi != r.edi){
    printf("EDI differ @ 0x%8x!\tnemu:\t%8x\tgt:\t%8x\n", cpu.eip, cpu.edi, r.edi);
    diff=true;
  }
  if(cpu.eip != r.eip){
    printf("EIP differ @ 0x%8x!\tnemu:\t%8x\tgt:\t%8x\n", cpu.eip, cpu.eip, r.eip);
    diff=true;
  }
