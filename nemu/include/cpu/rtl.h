...
static inline void rtl_sext(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  ...
  switch(width){
  case 1: 
    *dest = (int32_t)(int16_t)((*src1) & 0xFF); break;
  case 2:
    *dest = (int32_t)(int16_t)((*src1) & 0xFFFF); break;  //to do sign extention, src1 must be converted from uint16 to int16
  case 4:
    *dest = (int32_t)((*src1) & 0xFFFFFFFF); break;
  default:
    assert(0);
  }
}
...
static inline void rtl_update_ZF(const rtlreg_t* result, int width) {
  ...
  switch(width){
  case 1: 
    cpu.eflags.ZF = !((*result) & 0xFF); break;
  case 2:
    cpu.eflags.ZF = !((*result) & 0xFFFF); break;
  case 4:
    cpu.eflags.ZF = !((*result) & 0xFFFFFFFF); break;
  default:
    assert(0);
  }
  // printf("Here0!%d\n", cpu.eflags.ZF);
...
static inline void rtl_load_cr(rtlreg_t* dest, int num) {
  if(num == 0){
    *dest = cpu.cr0;
  }
  else if(num == 3){
...
static inline void rtl_store_cr(int num, const rtlreg_t* src) {
  if(num == 0){
    cpu.cr0 = *src;
  }
  else if(num == 3){
